...
static JSValue athena_setCurrentDirectory(JSContext *ctx, JSValueConst *argv) {
    ...
    if (!path)
        return JS_ThrowSyntaxError(
            ctx, "Argument error: System.currentDirectory(file) takes a filename as string as argument.");
...
static JSValue athena_dir(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (!strcmp(path, "mc0:") || !strcmp(path, "mc1:")) {
        ...
        if (strlen(mcPath) == 0) strcpy((char *) mcPath, (char *) "/");
        ...
        for (int i = 0; i < numRead; i++) {
            ...
            JS_DefinePropertyValueStr(ctx, obj, "name", JS_NewString(ctx, (const char *) mcEntries[i].EntryName),
                                      JS_PROP_C_W_E);
            JS_DefinePropertyValueStr(ctx, obj, "size", JS_NewUint32(ctx, mcEntries[i].FileSizeByte), JS_PROP_C_W_E);
            JS_DefinePropertyValueStr(ctx, obj, "dir", JS_NewBool(ctx, (mcEntries[i].AttrFile & MC_ATTR_SUBDIR)),
                                      JS_PROP_C_W_E);
...
static JSValue athena_createDir(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (!path)
        return JS_ThrowSyntaxError(
            ctx, "Argument error: System.createDirectory(directory) takes a directory name as string as argument.");
    mkdir(path, 0777);
...
static JSValue athena_removeDir(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (!path)
        return JS_ThrowSyntaxError(
            ctx, "Argument error: System.removeDirectory(directory) takes a directory name as string as argument.");
    rmdir(path);
...
static JSValue athena_movefile(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (!path)
        return JS_ThrowSyntaxError(
            ctx, "Argument error: System.removeFile(filename) takes a filename as string as argument.");
    const char *oldName = JS_ToCString(ctx, argv[0]);
    const char *newName = JS_ToCString(ctx, argv[1]);
    if (!oldName || !newName)
        return JS_ThrowSyntaxError(
            ctx, "Argument error: System.rename(source, destination) takes two filenames as strings as arguments.");
...
static JSValue athena_copyfile(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (!ogfile || !newfile)
        return JS_ThrowSyntaxError(
            ctx, "Argument error: System.copyFile(source, destination) takes two filenames as strings as arguments.");
...
static JSValue athena_getDiscType(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    for (iz = 0; DiscTypes[iz].name[0]; iz++)
        if (DiscTypes[iz].type == discType) DiscType_ix = iz;
