...
static JSValue athena_image_draw(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (image->angle != 0.0) {
        drawImageRotate(&(image->tex), x, y, image->width, image->height, image->startx, image->starty, image->endx,
                        image->endy, image->angle, image->color);
    } else {
        drawImage(&(image->tex), x, y, image->width, image->height, image->startx, image->starty, image->endx,
                  image->endy, image->color);
    }
...
static JSValue athena_image_set_uint(JSContext *ctx, JSValueConst this_val, JSValue val, int magic) {
    ...
    uint32_t value;
    if (!s) return JS_EXCEPTION;
    if (JS_ToUint32(ctx, &value, val)) return JS_EXCEPTION;
