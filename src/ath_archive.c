...
static void create_dir(char *pathname, int mode) {
    ...
    if (r != 0) printf("Could not create directory %s\n", pathname);
}
...
static int verify_checksum(const char *p) {
    int n, u = 0;
    for (n = 0; n < 512; ++n) {
        if (n < 148 || n > 155) /* Standard tar checksum adds unsigned bytes. */
            u += ((unsigned char *) p)[n];
        else
            u += 0x20;
    }
...
static const JSCFunctionListEntry module_funcs[] = {
    JS_CFUNC_DEF("open", 1, athena_archiveopen),      JS_CFUNC_DEF("list", 1, athena_archiveget),
    JS_CFUNC_DEF("extractAll", 1, athena_extractall), JS_CFUNC_DEF("close", 1, athena_archiveclose),
