...
void sound_resume(Sound *snd) {
    if (stream_paused) {
        ...
        if (cur_snd->type == WAV_AUDIO) WakeupThread(wavThreadId);
...
static int sound_get_adpcm_duration(audsrv_adpcm_t *sample) {
    ...
    if (duration_ms == 0) duration_ms = sample->size / 47;
...
int sound_get_duration(Sound *snd) {
    ...
    if (snd->type == OGG_AUDIO) {
        ...
        if (snd == cur_snd) sound_resume(snd);
...
static void oggIoThread(void *arg) {
    ...
    do {
        if (!stream_paused && cur_snd->type == OGG_AUDIO) {
            ...
            while ((wrPtr + partsToRead < STREAM_RING_BUFFER_COUNT) && (PollSema(inSema) == inSema)) partsToRead++;
            ...
            for (i = 0; i < partsToRead; i++) SignalSema(outSema);
...
void sound_deinit(void) {
    ...
    if (ogg_started) {
        ...
        oggDeinit();
        if (cur_snd->type == OGG_AUDIO) free(cur_snd->fp);
        ogg_started = false;
    }

    if (wav_started) {
        if (stream_paused && cur_snd->type == WAV_AUDIO) WakeupThread(wavThreadId);
        ...
        DeleteThread(wavThreadId);
        if (cur_snd->type == WAV_AUDIO) fclose(cur_snd->fp);
        wav_started = false;
    }
