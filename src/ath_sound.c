...
static JSValue athena_play(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    switch (snd->type) {
        ...
        case OGG_AUDIO:
            ...
            break;
        case ADPCM_AUDIO:
            if (argc == 2) JS_ToInt32(ctx, &slot, argv[1]);
...
static JSValue athena_pause(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    Sound *snd;
    JS_ToUint32(ctx, &snd, argv[0]);
    if (snd->type != ADPCM_AUDIO) sound_pause();
    return JS_UNDEFINED;
}
...
static JSValue athena_resume(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    Sound *snd;
    JS_ToUint32(ctx, &snd, argv[0]);
    if (snd->type != ADPCM_AUDIO) sound_resume(snd);
    return JS_UNDEFINED;
}

static const JSCFunctionListEntry module_funcs[] = {
    JS_CFUNC_DEF("setVolume", 2, athena_setvolume), JS_CFUNC_DEF("load", 1, athena_load),
    JS_CFUNC_DEF("play", 2, athena_play),           JS_CFUNC_DEF("free", 1, athena_free),
    JS_CFUNC_DEF("deinit", 0, athena_deinit),       JS_CFUNC_DEF("isPlaying", 0, athena_isplaying),
    JS_CFUNC_DEF("duration", 1, athena_duration),   JS_CFUNC_DEF("repeat", 1, athena_repeat),
    JS_CFUNC_DEF("pause", 1, athena_pause),         JS_CFUNC_DEF("resume", 1, athena_resume),
};
