...
static int ethGetDHCPStatus(void) {
    ...
    if ((result = ps2ip_getconfig("sm0", &ip_info)) >= 0) {  // Check for a successful state if DHCP is enabled.
...
static int ethApplyIPConfig(int use_dhcp, const struct ip4_addr *ip, const struct ip4_addr *netmask,
    ...
    if ((result = ps2ip_getconfig("sm0", &ip_info)) >= 0) {
        ...
        if ((use_dhcp != ip_info.dhcp_enabled) ||
            (!use_dhcp && (!ip_addr_cmp(ip, (struct ip4_addr *) &ip_info.ipaddr) ||
                           !ip_addr_cmp(netmask, (struct ip4_addr *) &ip_info.netmask) ||
                           !ip_addr_cmp(gateway, (struct ip4_addr *) &ip_info.gw) || !ip_addr_cmp(dns, dns_curr)))) {
            if (use_dhcp) {
...
static JSValue athena_nw_requests_post(JSContext *ctx, JSValue this_val, int argc, JSValueConst *argv) {
    ...
    if (curl) {
        ...
        res = curl_easy_perform(curl);
        /* Check for errors */
        if (res != CURLE_OK) JS_ThrowSyntaxError(ctx, "curl_easy_perform() failed: %s\n", curl_easy_strerror(res));
